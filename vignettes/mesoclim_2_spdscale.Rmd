...
Coarse resolution climate and ancillary data (including sea surface temperature data where appropriate) will need to be prepared as outlined in `vignette('mesoclim_1_preparedata', package='mesoclim')`.  

Here we load package data previously saved using `write_climdata` function as .Rds files using the`read_climdata`functions. 

Sea surface data is also required for downscaling and in this example we use the package data derived from Tinker et al (2024).
...
t0<-now()
mesoclimate<-spatialdownscale(climdata=climdata, sst=sst, dtmf, dtmm, basins = NA,  wca=NA, skyview=NA, horizon=NA, cad=TRUE, coastal = TRUE, thgto =2, whgto=2, include_tmean=TRUE, rhmin = 20, pksealevel = FALSE, patchsim = FALSE,  terrainshade = TRUE, precipmethod = "Elev", fast = TRUE, noraincut = 0.01, toArrays=FALSE) # 
print(now()-t0)
...
Example below downscales to the whole of the Lizard for just a single month using small tiles for demonstration purposes. Run time may be up to 3-4 mins, so this code is not run when building this vignette.

```{r downscale_by_tiles, eval=FALSE, out.width="50%"}
...
tmeanc<-(climdata$tmin+climdata$tmax)/2
...
plot(lwrad[[13]],main=paste("LW downward radiation",terra::time(lwrad)[[13]]))
```
...
The `precipdownscale()` function provides various options and methods to downscale rainfall. Here we apply a correction based upon local elevation data.
...
plot(precelev[[13]],main=paste("Precipitation",terra::time(precelev)[[13]]))
```

## References

Tinker et al. (2024) Twenty-first century marine climate projections for the NW European shelf seas based on a perturbed parameter ensemble. Ocean Science 20:3 835--885. DOI: 10.5194/os-20-835-2024 https://os.copernicus.org/articles/20/835/2024/
