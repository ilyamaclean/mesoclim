...
Bias correction compares modeled historic data with observational data for the same period to directly correct modelled data and/or provide bias-correction models for correction of modelled data. This requires comparable observed and modeled data for the same historic period and geographical area. 

Once bias-correction models have been calculated for the historic period, they can then be applied to correct modelled data for the same geographical area but for future time periods.
...
Here we bias correct one month of future temperature and precipitation data and plot the mean and standard deviation difference and corrected mean values. 

```{r future_data}
futuredata<-read_climdata(mesoclim::ukcpfuture)
msk<-ifel(lsm12km.r==0,NA,lsm12km.r) # for use in cropping and masking sea cells

bcdata<-list()
for (v in c("tmax","tmin")) {
  bcdata[[v]]<-biascorrect_apply(fut_mod=crop(futuredata[[v]],msk,mask=TRUE), biasmods=model_list[[v]], rangelims = NA)
  # Calculate difference between future input data and bias corrected future data
  dif<-mask(futuredata[[v]],msk)-bcdata[[v]]
  stats<-c(app(dif,mean),app(dif,sd),mean(bcdata[[v]]))
  names(stats)<-paste(v,c("mean difference (input-bc)","sd difference (input-bc)","mean corrected values"))
  plot(stats)
}
