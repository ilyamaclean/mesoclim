...
for(v in c('tmin','tmax','relhum','pres','swrad','lwrad','windspeed','winddir','prec')) plot(.rast(ukcpinput[[v]],ukcpinput$dtm)[[1]],main=v)
```
...
Using pre-calculated bias correction models.

```{r bias_correction}
# Get pre-calculated models
dir_bmodels<-"/Users/jonathanmosedale/Library/CloudStorage/OneDrive-UniversityofExeter/Data/bias_correct_models"
model_list<-readRDS(file.path(dir_bmodels,paste0('bias_correct_models_2020_2022.Rds')))

# Apply  models
climdata<-biascorrect_climdata(ukcpinput,model_list,prec_thold =0.01)
