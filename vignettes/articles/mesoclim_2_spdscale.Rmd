...
sst<-rast(mesoclim::ukcp18sst)
dtmc<-unwrap(climdata$dtm)
...
```{r onestep-spdown}
# Downscale one month of  climate data
...
t0<-now()
mesoclimate<-spatialdownscale(climdata=climdata, sst=ukcp18sst, dtmf, dtmm, basins = basins,  wca=wca, skyview=skyview, horizon=horizon, cad=TRUE, coastal = TRUE, thgto =2, whgto=2, rhmin = 20, pksealevel = TRUE, patchsim = FALSE,  terrainshade = TRUE, precipmethod = "Elev", fast = TRUE, noraincut = 0.01, toArrays=FALSE) # CHECK UKCP pres treated as sea level
print(now()-t0)
...
```{r wind_dscale}
windspeed<-winddownscale(climdata$windspeed, ukcpinput$winddir, dtmf, dtmm, ukcpinput$dtm, zi = ukcpinput$windheight_m, zo=2)
...
```{r temp-dscale}
t0<-now()
dailytemps<-tempdaily_downscale( climdata, 
                                 NA, # or precalculate mean temp
                                ukcp18sst,# can be coarse resolution
                                dtmf, # fine scale dem
                                dtmm  , # medium re wider are dem
                                basins = basins, # basindelin() output or will calculate
                                uzf = windspeed, # windspeeds downscaled - or will calculate
                                cad = TRUE,
                                coastal = TRUE,
...
# Spatial downscaling across large areas using tiles

An alternative function is available for downscaling across large areas. The process divides the area into several tiles for which downscaling is then performed monthly. Default size of tiles is 10km^2 but this can be modified by the parameters. 

Example below downscales to the whole of the Lizard for just a single month using small tiles for demonstration purposes.

```{r downscale_by_tiles}
dtmf<-rast(system.file("extdata/dtms/lizard50m.tif",package="mesoclim"))
dtmm<-rast(system.file("extdata/dtms/dtmm.tif",package="mesoclim"))
sstdata<-mesoclim::ukcp18sst
climdata<-mesoclim::ukcpinput
