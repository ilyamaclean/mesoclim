...
As with spatial downscaling temporal downscaling of all daily variables can be performed by the wrapper function `temporaldownscale`. The function assumes `tmax` and `tmin` daily values are available.

```{r singlestep_tempdscale, fig.height=8}
...
**Another option is to downscale from LWnet by removing temperature dependent LWup using hourly temps????**
